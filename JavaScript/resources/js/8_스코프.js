/*
    스코프
    - 변수, 함수를 접근 할 수 있는 유효 범위
    - 값을 찾을 때 : 내부 -> 외부 -> 전역 순으로 거슬러 올라가면서 찾음 (스코프 체인)

    종류
    - 전역 스코프 : 코드 어디서든 접근 가능
    - 함수 스코프 : 함수 내부에서만 접근 가능 (var)
    - 블록 스코프 : {}블로개 내부에서만 접근이 가능 (let, const)
    - 렉시컬 스코프 : "선언된 위치"기준으로 스코프를 결정(js)
*/
// 기본적으로 함수 내부에 값이 없으면 전역에서 값을 찾는다.
var num1 = 20;
function test1 () {
    console.log(num1);  // 전역에 있는 num1: 20
}

function test2 () {
    var num1 = 40;
    console.log(num1);  // 내부에 있는 num1: 40
}
...
/*
    ...
    (실행 위치 기준은 동적 스코프 -> js는 동적 스코프가 아니다.)
*/
