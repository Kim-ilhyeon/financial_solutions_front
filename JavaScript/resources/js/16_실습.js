...
console.log("1. 정답 : " + numbers);

// 문제 2. 문자열을 받아 거꾸로 뒤집은 문자열을 반환하는 함수 reverseStr(str) 을 작성하세요.
// (배열의 reverse() 사용 금지, 반복문으로 직접 구현)
...
// 문제 3. 양의 정수를 받아 각 자리 숫자의 합을 반환하는 함수 digitSum(n) 을 작성하세요.
// (반복문과 %, 나눗셈 활용)
function digitsum(n) {
    let sum = 0;
    let str = String(n);
    console.log(typeof str);    // 문자열로 변환되었는지 확인
    for (const st of str) {
        sum += Number(st);
    }
    return sum;
}
console.log("3. 정답(풀이1) : " + digitsum(1234));     // 10 (1+2+3+4)
...
function digitsum2(n) {
    ...
    while (n > 0) {
        sum2 += n % 10; //1의자릿수 누적
        n = Math.floor(n / 10); // 자릿수 줄여주기
    }
    return sum2;
}
digitsum2(1234);
console.log("3. 정답(풀이2) : " + digitsum2(1234))

// 문제 4. 아래 코드의 출력을 "실행하기 전에" 예측해 주석으로 적으세요.
console.log(typeof a);  // Number 출력 예상 (X) -> Undefined 출력됨 
// -> 선언 전에 a변수를 사용하면 호이스팅으로 인해 에러는 나지 않지만 선언만 된상태라서 Undefined가 출력됨
var a = 5;
...
const fn = outer();
fn();       // 2 출력 예상 (O) -> 선언 위치의 x를 참조(렉시컬 스코프)
x = 100;
fn();       // 2 출력 예상 (O) -> 전역 x의 값과 fn함수에서 출력하는 값은 무관하다.
// fn함수에서 출력하는 x는 선언 시점에 클로저로 저장된 값이다.

// 문제 5. 호출할 때마다 "user_1", "user_2", "user_3" … 처럼
// 1씩 증가하는 아이디를 반환하는 함수 makeIdGenerator() 를 클로저로 작성하세요. 
// (내부 카운터는 외부에서 접근 불가)
...
// 문제 6. 함수 fn 과 횟수 n 을 받아, fn 을 n 번 실행하는 고차함수 repeat(fn, n) 을 
// 작성하세요. fn 에는 현재 회차(0부터)가 인자로 전달되도록 하세요.
...
// (filter + map 활용)
const upPrice = products.filter(pd => pd.price>=100000).map(pd => pd.name);
console.log("7. 정답 : " + upPrice);
...
const avgPrice = products.reduce((acc, cur) => {
    acc += cur.price;
    let result = acc / products.length;
    return result;
}, 0);

console.log("8. 정답(합계) : " + sumPrice);
console.log("8. 정답(평균) : " + avgPrice);
console.log("8. 정답(평균) : " + sumPrice/products.length);

// 문제 9. 가장 비싼 상품의 객체를 찾아 출력하세요. (Math.max 없이 reduce 또는 반복문으로)
let maxPrice = products.reduce((acc, cur) => {
    return acc.price < cur.price ? cur : acc;
}, products[0]);

console.log("9.정답(풀이1) : ");
...
console.log("9. 정답(풀이2) : ");
console.log(max);

// 문제 10. 아래 두 객체를 병합하되, 같은 key는 update 의 값이 우선되도록
// 스프레드로 새 객체 merged 를 만드세요. 그 후 구조분해로 name 과 age 만 추출해 출력하세요.
...
// 문제 11. 문자열 배열을 받아 
// 각 요소(단어)의 등장 횟수를 객체로 집계하는 함수 countWords(arr) 를 작성하세요.
// { a: 3, b: 2, c: 1 }
...
// 문제 12. 아래 origin 객체를 JSON 문자열로 변환했다가 다시 객체 copy 로 복원하세요.
...
console.log(stdList);
// 2. 총점 기준 내림차순으로 정렬한다.
stdList.sort((a, b) => b.total = a.total);  // 내림차순
console.log(stdList);
...
const avg = stdList.reduce((acc, cur) => {
    acc += cur.total;
}, 0) / stdList.length;
stdList
    .filter((std) => std.total >= avg)
    .forEach((std) => console.log(std.name));

// **문제 15.** `createGradeBook()` 함수를 작성하세요. 
// 클로저를 이용해 내부에 학생 목록을 private하게 보관하고, 다음 메서드를 
// 가진 객체를 반환합니다. (내부 학생 배열은 외부에서 직접 접근할 수 없어야 합니다.)
...
function createGradeBook() {
    ...
    function getRankList() {
        const sortList = stdList
                            ...
                            .sort((a, b) => b.total - a.total); // 총점 내림차순 정렬
        return sortList;
    }
